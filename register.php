...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($idNo) || empty($email) || empty($firstName) || empty($lastName) || empty($password) || empty($confirm)) {
        ...
        if ($check->num_rows > 0) {
            ...
            $stmt = $conn->prepare("INSERT INTO users (idNo, email, FirstName, LastName, password, role) VALUES (?, ?, ?, ?, ?, ?)");
            $stmt->bind_param("ssssss", $idNo, $email, $firstName, $lastName, $hashedPassword, $role);
